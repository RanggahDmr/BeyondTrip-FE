...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onTripCreated?: (trip: any) => void; // ✅ callback ke dashboard
};
...
export default function CreateAi({ isOpen, onClose, onTripCreated }: Props) {
  ...
  // Vision state (tetap dipertahankan)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const routeText: string = routeRes.data.route;
      setResult(routeText);

      // 2️⃣ Simpan trip ke database
      ...
      const newTrip = saveRes.data.trip;

      // 3️⃣ Beri tahu dashboard (optimistic update)
  ...
  // Guess destination from image
  const handleGuessFromImage = async () => {
    try {
      if (!destImageFile) {
        toast.error("Upload a destination photo first");
        return;
      }
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || ""}/api/guess-destination`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${session.access_token}` },
          body: fd,
      ...
      const auto = [g?.name, g?.city, g?.region, g?.country]
        .filter(Boolean)
        .join(", ");
      ...
      toast.success(
        g?.name
          ? `Detected: ${g.name}${g.city ? ", " + g.city : ""}`
          : "Detected — review the suggestion"
      );
    } catch (e) {
      console.error(e);
      toast.dismiss();
        ...
        >
          ...
          >
            ...
            <h1 className="text-3xl font-bold mb-6 text-center">
              Generate AI Travel Route
            </h1>

            {/* Form */}
            <form
              onSubmit={handleSubmit}
              className="bg-white/10 p-6 rounded-2xl backdrop-blur-md w-full space-y-4 border border-white/20"
            >
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg bg:white/10 bg-white/10 border border-gray-500 focus:outline-none"
              ...
              <div>
                ...
                >
                  <option className="text-black" value="mobil">
                    Mobil
                  </option>
                  ...
                  <option className="text-black" value="tanpa kendaraan">
                    Transportasi umum
                  </option>
                </select>
              </div>
